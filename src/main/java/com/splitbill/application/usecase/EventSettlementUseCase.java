...
public class EventSettlementUseCase {
    ...
    @Transactional
    public List<EventSettlementResponse> listByEvent(UUID eventId) {
    ...
    @Transactional
    public EventSettlementResponse updateStatus(UUID settlementId, UpdateSettlementStatusRequest request) {
        ...
        UserJpaEntity admin = request.adminUserId() == null
                ? users.findAll().stream()
                        .filter(UserJpaEntity::isAdmin)
                        .findFirst()
                        .orElse(null)
                : users.findById(request.adminUserId())
                        .orElseThrow(() -> new DomainException("Admin user not found"));
        if (admin != null && !admin.isAdmin()) {
            throw new DomainException("Only admin users can update settlement status");
        }
